    ...
    async def list(
        ...
        *,
        ...
        if collection_id is not None:
            from src.collections.service import ensure_collection_owner

            await ensure_collection_owner(session, user_id=user_id, collection_id=collection_id)
        if collection_id is not None:
            documents = await DocumentRepository.from_session(session).get_by_user_id(
                user_id,
...
def knowledge_gap_result(
    *,
    ...
    return KnowledgeGap(
        ...
        why_detected=summary_why_detected(
            topic=topic,
            coverage_ratio=coverage_ratio,
            missing_count=missing_count,
            document_count=len(documents),
...
def topic_document_from_document(document: DocumentModel) -> TopicDocumentRecord:
    from src.topics.repository import TopicDocumentRecord
...
def to_note_response(dto: DocumentNoteResponse) -> NoteResponse:
    return NoteResponse(
        id=dto.id,
...
knowledge_gaps = KnowledgeGapService()

__all__ = [
    "KnowledgeGapService",
    "collection_topic_gaps",
