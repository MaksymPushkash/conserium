...
async def run_due_repo_syncs_task_impl() -> dict[str, int]:
    try:
        return await run_due_repo_syncs()
    except Exception as exc:
        logger.exception("Scheduled repo sync failed: %s", exc)
        return {"queued": 0, "completed": 0, "failed": 1}


async def run_repo_sync_task_impl(*, user_id: str, repo_sync_id: str, max_files: int = 50) -> dict[str, object]:
    try:
...
async def run_drain_repo_sync_outbox_task(limit: int = 100) -> dict[str, int]:
    try:
        return await drain_repo_sync_outbox(limit=limit)
    except Exception as exc:
