...
from src.infrastructure.celery.composition import process_audio_document as process_audio_document_use_case
...
@celery_app.task(  # type: ignore[untyped-decorator]
    name="src.infrastructure.celery.tasks.media_processing_tasks.process_audio_document",
...
def process_audio_document(self: Any, document_id: str) -> dict[str, str]:
    ...
    log.info("Media audio processing task started")

    try:
        result = run_worker_async(process_audio_document_use_case(document_id))
        log.info("Media audio processing task completed", status=result["status"])
        return result
    except Exception as exc:
        log.exception("Media audio processing task failed", error=str(exc))
