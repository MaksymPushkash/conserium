...
class _FakeAudioExtractor(IContentExtractor):
    async def extract_from_bytes(
        ...
        *,
        ...
        return ExtractedContent(text="transcribed speech", title="voice", word_count=2)

    async def extract_from_url(self, url: str) -> ExtractedContent:
        raise NotImplementedError


def _make_audio_document() -> DocumentEntity:
...
@pytest.mark.asyncio
async def test_process_audio_document_use_case_transcribes_and_dispatches_embeddings() -> None:
    document = _make_audio_document()
    ...
    assert status_cache.calls == [
        ("PROCESSING", 20, "Transcribing audio..."),
        ("PROCESSING", 40, "Splitting transcript into chunks..."),
        ("PROCESSING", 60, "Queued for embedding..."),
    ]
    assert task_dispatcher.embed_calls == [
        {
            "document_id": str(document.id),
            "raw_text": "transcribed speech",
            "chunks_data": [
                {
