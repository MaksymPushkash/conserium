...
from src.application.ports.persistence.telegram_repository import TelegramChatBindingRecord, TelegramPairingCodeRecord
...
async def test_telegram_pairing_code_consumes_and_creates_binding() -> None:
    ...
    created_code = await CreateTelegramPairingCodeUseCase(cast("IUnitOfWork", uow))(user_id=user_id)
    binding = await ConsumeTelegramPairingCodeUseCase(cast("IUnitOfWork", uow))(
        ConsumeTelegramPairingCodeDTO(
    ...
    status = await GetTelegramStatusUseCase(cast("IUnitOfWork", uow))(user_id=user_id)

    assert binding.user_id == user_id
    assert binding.chat_id == "123"
    assert status.bindings == [binding]
    assert uow.telegram_repo.pairing_codes[0].consumed_at is not None
    assert uow.api_key_repo.records[0].scopes == ["ingest:write", "status:read"]
...
async def test_telegram_ingest_uses_chat_binding_user() -> None:
    ...
    api_key_id = uuid4()
    uow = _TelegramUow()
    uow.telegram_repo.bindings.append(_binding(user_id=user_id, api_key_id=api_key_id, chat_id="123"))
    ingest = _RecordingExternalIngest()

    await IngestTelegramItemUseCase(cast("IUnitOfWork", uow), cast("IngestExternalItemUseCase", ingest))(
        chat_id="123",
...
async def test_revoke_telegram_binding_revokes_api_key() -> None:
    ...
    uow = _TelegramUow()
    uow.telegram_repo.bindings.append(binding)
    uow.api_key_repo.records.append(_api_key(user_id=user_id, api_key_id=api_key_id))

    await RevokeTelegramBindingUseCase(cast("IUnitOfWork", uow))(user_id=user_id, binding_id=binding.id)

    assert uow.telegram_repo.bindings[0].revoked_at is not None
    assert uow.api_key_repo.records[0].revoked_at is not None
...
class _TelegramUow:
    ...
    async def __aenter__(self) -> _TelegramUow:
        return self
