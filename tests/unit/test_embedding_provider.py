...
def test_openai_embedding_provider_rejects_mismatched_dimension_config() -> None:
    with patch(
        "src.kit.ai.providers.openai_embedding_provider.settings.OPENAI_EMBEDDING_DIMENSIONS",
        ChunkModel.EMBEDDING_DIMENSIONS + 1,
...
async def test_openai_embedding_provider_requires_api_key() -> None:
    with patch("src.kit.ai.providers.openai_embedding_provider.settings.OPENAI_API_KEY", ""):
...
async def test_openai_embedding_provider_configures_client_timeout_and_retries() -> None:
    with (
        patch("src.kit.ai.providers.openai_embedding_provider.settings.OPENAI_API_KEY", "test-key"),
        patch("src.kit.ai.providers.openai_embedding_provider.AsyncOpenAI") as client_cls,
    ):
        client = AsyncMock()
        client.embeddings.create.return_value.data = [
            type("Embedding", (), {"embedding": [0.1] * ChunkModel.EMBEDDING_DIMENSIONS})()
        ]
...
def test_startup_validation_rejects_mismatched_embedding_dimensions() -> None:
    with patch(
        "src.startup_checks.settings.OPENAI_EMBEDDING_DIMENSIONS",
        ChunkModel.EMBEDDING_DIMENSIONS + 1,
...
def test_startup_validation_requires_openai_key_when_not_debug() -> None:
    with (
        patch("src.startup_checks.settings.DEBUG", False),
        patch("src.startup_checks.settings.OPENAI_API_KEY", ""),
...
def test_startup_validation_rejects_unknown_file_storage_type() -> None:
    with (
        patch("src.startup_checks.settings.DEBUG", True),
        patch("src.startup_checks.settings.FILE_STORAGE_TYPE", "ftp"),
...
def test_startup_validation_requires_bucket_for_s3_storage() -> None:
    with (
        patch("src.startup_checks.settings.DEBUG", True),
        patch("src.startup_checks.settings.FILE_STORAGE_TYPE", "s3"),
        patch("src.startup_checks.settings.AWS_S3_BUCKET", ""),
